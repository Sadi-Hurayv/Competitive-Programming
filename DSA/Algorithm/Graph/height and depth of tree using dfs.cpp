...
void dfs(int vertex, vector<vector<int>> &tree, int parent, vector<int> &heights, vector<int> &depths)
{
    // visited[vertex] = true;
    ...
    for (auto adjacent : tree[vertex])
    {
        ...
        dfs(adjacent, tree, vertex, heights, depths);

        heights[vertex] = max(heights[vertex], heights[adjacent] + 1);
    }
