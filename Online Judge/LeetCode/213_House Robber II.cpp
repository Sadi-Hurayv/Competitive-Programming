...
int rob(vector<int> &money)
{
    ...
    // vector<int> memo1(temp1.size() + 1, -1);
    // vector<int> memo2(temp2.size() + 1, -1);
    // return max(momoization(temp1.size() - 1, temp1, memo1), momoization(temp2.size() - 1, temp2, memo2));

    return max(iteration(temp1.size() - 1, temp1), iteration(temp2.size() - 1, temp2));
}
