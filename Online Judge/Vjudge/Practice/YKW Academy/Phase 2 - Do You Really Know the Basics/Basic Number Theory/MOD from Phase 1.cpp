#include <bits/stdc++.h>
using namespace std;

typedef long long ll;
typedef unsigned long long ull;

// For Loop
#define fi(i, a, c) for (ll i = (a); i < (c); i++)
#define fd(i, a, c) for (ll i = (a); i > (c); i--)
#define fe(it, a) for (auto it : a)

// Pair
#define pi pair<int, int>
#define pll pair<ll, ll>
#define ff first
#define ss second

// Tuple
#define tll tuple<ll, ll, ll>

// Vector
#define vi vector<int>
#define vll vector<ll>
#define vd vector<double>
#define vpi vector<pi>
#define vpll vector<pll>
#define vtll vector<tll>
#define vvi vector<vi>
#define vvll vector<vll>

// STL Functions Related
#define pub push_back
#define pob pop_back
#define puf push_front
#define pof pop_front
#define mp make_pair
#define in insert
#define lb lower_bound
#define ub upper_bound
#define all(a) a.begin(), a.end()

// Constants
#define MOD 1000000007
#define PI 3.141592653589793238462
#define EPS 1e-6
#define PINF 1e18
#define NINF -1e18

// Others
#define endl "\n"

//________________________________________________________________________________________________________________ \\


ll modular_multiplication(ll a, ll b, ll mod)
{
    return ((__int128(a) % mod) * (b % mod)) % mod;
}

ll power(ll a, ll n, ll mod)
{
    if (n == 0)
        return 1 % mod;
    if (n == 1)
        return a % mod;
    ll x = power(a, n / 2, mod);
    if (n % 2)
        return modular_multiplication(modular_multiplication(x % mod, x % mod, mod) % mod, (a % mod), mod); // return ((((x % mod) * (x % mod)) % mod) * (a % mod)) % mod;
    else
        return modular_multiplication(x % mod, x % mod, mod); // return (((x % mod) * (x % mod)) % mod);
}

ll inverse_mod(ll a, ll mod)
{
    return power(a, mod - 2, mod) % mod;
}

ll modular_division(ll a, ll b, ll mod)
{
    return modular_multiplication(a, inverse_mod(b, mod), mod);
}

int main()
{
    ios_base::sync_with_stdio(false);
    cin.tie(NULL), cout.tie(NULL);

#ifndef ONLINE_JUDGE
    freopen("/media/shuvra/New Volume/IIT/Programming/Competitive-Programming/IO/input.txt", "r", stdin);
    freopen("/media/shuvra/New Volume/IIT/Programming/Competitive-Programming/IO/output.txt", "w", stdout);
#endif // ONLINE_JUDGE

    ll a, b, p;
    bool ok, flg;

    cin >> a >> b >> p;

    cout << power(a, b, p) << endl;
    cout << modular_multiplication(a, b, p) << endl;
    cout << modular_division(a, b, p) << endl;

    return 0;
}